...
mkdir outs
cd outs
mkdir "02" "04" "05" "07" "08" "07B" "08B" "09B"
cd ../
# TES-01
...
echo -e "${BOLD}${BLUE}## TEST-08${RESET}"
PATH_OUTS='./outs/08'

ORG_CMD="$( < infile /srcs/a.out  | ls  > $PATH_OUTS/output-original.txt)"
echo EXIT STATUS : $? > "$PATH_OUTS/status-my.txt"



PIPEX_CMD="$(./pipex infile /srcs/a.out  ls  $PATH_OUTS/output-my.txt)"
echo EXIT STATUS : $? > "$PATH_OUTS/status-org.txt"
...
echo -e "${BOLD}${BLUE}## TEST-07B${RESET}"
PATH_OUTS='./outs/07B'
...
# TEST08B
#  < exist_input /srcs/a.out  | ls        

echo -e "${BOLD}${BLUE}## TEST-08B${RESET}"
PATH_OUTS='./outs/08B'
...
# TEST09B
#   < exist_input /bin/caqwdt  | /bin/lsqwd -ls > nodir/outfiile             

echo -e "${BOLD}${BLUE}## TEST-09B${RESET}"
PATH_OUTS='./outs/09B'

ORG_CMD="$( < exist_input /bin/caqwdt  | /bin/lsqwd -ls > nodir/outfiile > randomfolder/output-original.txt)"
echo EXIT STATUS : $? > "$PATH_OUTS/status-my.txt"



PIPEX_CMD="$(./pipex infile  /bin/caqwdt "/bin/lsqwd -ls"  $PATH_OUTS/dd/output-my.txt)"
echo EXIT STATUS : $? > "$PATH_OUTS/status-org.txt"
